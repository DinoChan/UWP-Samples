...
namespace ColorWheelDemoSilverlight
{
    ...
    public struct RgbColor
    {
        ...
        public RgbColor(int R, int G, int B)
        {
            ...
            if (R < 0 || R > 255)
            {
                throw new ArgumentOutOfRangeException("R","R must corrospond to a byte value");
            }
            if (G < 0 || G > 255)
            {
                throw new ArgumentOutOfRangeException("G", "G must corrospond to a byte value");
            }
            if (B < 0 || B > 255)
            {
                throw new ArgumentOutOfRangeException("B",  "B must corrospond to a byte value");
            }
#endif
            Red = R;
            Green = G;
